...
![fig1](AI/ML-DL/Compute_Graph-1.png){: style="display:block; margin:0 auto; width:60%;"}
...
$$
y=wx\in\mathbb{R}~~~~~~,~~x\in\mathbb{R}~,w\in\mathbb{R}
$$
...
$$
y=\mathbf{w}^\top\mathbf{x}\in\mathbb{R}~~~~~~,~~\mathbf{x}\in\mathbb{R}^n~,\mathbf{w}\in\mathbb{R}^n
$$
...
$$
\mathbf{y}=W\mathbf{x}\in\mathbb{R}^m~~~~~~,~~\mathbf{x}\in\mathbb{R}^n~,W\in\mathbb{R}^{m\times n}
$$
...
$$
Y=\mathbf{W}X\in\mathbb{R}^{m\times B}~~~~~~,~~X\in\mathbb{R}^{n\times B}~,\mathbf{W}\in\mathbb{R}^{(m\times B)\times(n\times B)}
$$
