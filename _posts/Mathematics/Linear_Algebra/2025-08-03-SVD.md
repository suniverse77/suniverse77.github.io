...
SVD는 EVD와 달리 어떠한 행렬이든 다음 3개의 행렬로 분해가 가능하다.
...
- $U$: 각 열이 $A$의 특이 벡터들로 구성된 직교 행렬

    $$
    U=\begin{bmatrix}|&|&&|\\\mathbf{v}_1&\mathbf{v}_2&\cdots&\mathbf{v}_n\\|&|&&|\end{bmatrix}
    \in\mathbb{R}^{m\times m}
    $$

- $\Sigma$: 주대각선에 $A$의 특이값들이 크기순으로 나열된 대각 행렬
...
SVD는 복잡해 보이는 변환 $A$를 고유벡터 좌표계에서는 훨씬 단순한 변환으로 볼 수 있다.
...
1. $V^\top\mathbf{x}$: 기존 좌표계의 벡터 $\mathbf{x}$를 고유벡터들을 기저로 하는 좌표계에서의 좌표로 바꾼다.

2. $\Sigma(V^\top\mathbf{x})$: 고유벡터 좌표계에서 각 성분을 해당 고유값 $\lambda_i$만큼 독립적으로 스케일링한다.

3. $U(\Sigma(V^\top\mathbf{x}))$: 스케일링된 결과를 다시 원래 좌표계로 되돌린다.
