...
SVD는 EVD와 달리 정사각 행렬이 아니어도 적용할 수 있으며, 임의의 행렬 $A \in \mathbb{R}^{m \times n}$을 다음 세 행렬의 곱으로 분해할 수 있다.
...
- $\Sigma$: 주대각선에 $A$의 **특이값**들이 크기순으로 나열된 대각 행렬

    $$
    \Sigma=\begin{bmatrix}\sigma_1&0&\cdots&0\\0&\sigma_2&\cdots&0\\\vdots&\vdots&\ddots&0\\0&0&\cdots&\sigma_n\\0&0&\cdots&0\end{bmatrix}
...
예를 들어 행렬 $A$의 SVD가 $A=U\Sigma V^\top$로 주어질 때, 벡터 $\mathbf{x} \in \mathbb{R}^n$에 대한 변환은 다음과 같이 쓸 수 있다.
...
1. $V^\top\mathbf{x}$: 기존 좌표계의 벡터 $\mathbf{x}$를 Right Singular Vector들을 기저로 하는 좌표계에서의 좌표로 바꾼다.

2. $\Sigma(V^\top\mathbf{x})$: Right Singular Vector 좌표계에서 각 성분을 표현된 각 성분을 해당 특이값 $\sigma_i$만큼 스케일링한다.

3. $U(\Sigma V^\top\mathbf{x})$: 스케일링된 결과를 Left Singular Vector들을 기저로 하는 출력 좌표계에서, 다시 표준 출력 좌표계로 표현한다.
...
<br>
또한 행렬 $A$는 다음과 같이 SVD가 된다고 가정해 보자.
...
1. 벡터 $\mathbf{x}$를 Right Singular Vector 좌표계로 표현
   ...
   즉, $\mathbf{x}$는 Right Singular Vector 좌표계에서 $2\mathbf{v}_1-2\mathbf{v}_2$로 표현된다.
...
2. 각 성분을 특이값 $\sigma_1=2,\sigma_2=0.5$만큼 Scaling
   ...
   즉, Scaling한 벡터는 Right Singular Vector 좌표계에서 $4\mathbf{v}_1-\mathbf{v}_2$로 표현된다.

3. Left Singular Vector 좌표계
   ...
   즉, $[4, -1]$는 Left Singular Vector 좌표계에서 $\mathbf{u}_1+4\mathbf{u}_2$로 표현된다.

즉, $A$에 의한 선형 변환의 결과는 다음과 같다.

$$
A\begin{bmatrix}2\\2\end{bmatrix}=\begin{bmatrix}1\\4\end{bmatrix}
$$
