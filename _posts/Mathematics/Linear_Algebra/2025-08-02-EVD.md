...
닮음 관계는 <span style="background-color:#fff5b1">행렬의 본질적인 성질은 유지하면서, 행렬을 다른 기저에서 표현하는 것</span>을 의미한다.
...
대각 행렬은 각 좌표를 독립적으로 스케일링하는 성질이 있다.
<br>
즉 EVD를 통해 행렬 $A=V\Lambda V^{-1}$로 분해하면, $A$라는 복잡한 변환이 <span style="background-color:#fff5b1">고유벡터 좌표계에서는 단순한 축별 스케일링으로 바뀌게 된다.</span>
...
이는 오른쪽부터 차례대로 다음의 3단계를 거치는 것과 같다.

1. $V^{-1}\mathbf{x}$: 기존 좌표계의 벡터 $\mathbf{x}$를 고유벡터들을 기저로 하는 좌표계에서의 좌표로 바꾼다.

2. $\Lambda(V^{-1}\mathbf{x})$: 고유벡터 좌표계에서 표현된 각 성분을 해당 고유값 $\lambda_i$만큼 독립적으로 스케일링한다.
...
+c_2\begin{bmatrix}3\\0\end{bmatrix}
\quad\to\quad
c_1=-1~,~c_2=1
$$

| 실제 좌표 | 고유벡터 좌표계에서의 표현 |
| :---: | :---: |
| $\begin{bmatrix}2\\\\2\end{bmatrix}$ | $-\mathbf{v}_1+\mathbf{v}_2$|
...
2. 각 성분을 고유값 $\lambda_1=-1,\lambda_2=0.5$만큼 Scaling
   ...
   Scaling 후 $\mathbf{v}_1+0.5\mathbf{v}_2$
