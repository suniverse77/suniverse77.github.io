...
1. $V^{-1}\mathbf{x}$
   ...
   기존 좌표계의 벡터 $\mathbf{x}$를 고유벡터들을 기저로 하는 좌표계에서의 좌표로 바꾼다.

2. $\Lambda(V^{-1}\mathbf{x})$

   고유벡터 좌표계에서 각 성분을 해당 고유값 $\lambda_i$만큼 독립적으로 스케일링한다.

3. $V(\Lambda V^{-1}\mathbf{x})$

   스케일링된 결과를 다시 원래 좌표계로 되돌린다.
...
다만 여기서 주의할 점은, 일반적인 EVD에서 고유벡터들이 반드시 서로 직교하는 것은 아니라는 점이다. 따라서 $V^{-1}$와 $V$는 단순한 회전 변환이 아니라, 일반적인 기저 변환으로 이해하는 것이 정확하다. 특히 $A$가 대칭 행렬이면 고유벡터들을 직교정규 기저로 잡을 수 있고, 이때는 좌표계 변환을 회전 또는 반사처럼 더 직관적으로 해석할 수 있다.
