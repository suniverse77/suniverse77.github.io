...
| 명령어 | 설명 | 사용 예시
| :---: | :---: | :---: |
| `docker pull` | 이미 만들어둔 이미지 다운로드 | `docker pull {이미지 이름}` |
| `docker build` | 도커파일을 이용해 이미지 빌드 | `docker build -t {빌드할 이미지 이름} {Dockerfile 경로}` |
| `docker images` | 현재 존재하는 도커 이미지들의 전체 목록과 용량 출력 |
| `docker rmi` | 사용하지 않는 도커 이미지 삭제 | `docker rmi {제거할 이미지 ID 또는 이름}` |
...
| 명령어 | 설명 | 사용 예시 |
| :---: | :---: | :---: |
| `docker run` | 이미지를 바탕으로 컨테이너 생성 및 실행 | `docker run {옵션} {사용할 이미지 이름}` |
| `docker ps` | 현재 실행 중인 컨테이너들의 목록, 상태, 생성 시간 등 출력 | `docker ps {옵션}` |
| `docker stop` | 실행 중인 컨테이너 중지 | `docker stop {컨테이너 ID 또는 이름}` |
| `docker rm` | 중지된 컨테이너를 내 컴퓨터에서 완전히 삭제 | `docker rm {옵션} {컨테이너 ID 또는 이름}` |
| `docker logs` | 컨테이너의 실시간 로그 출력 | `docker logs {옵션} {컨테이너 ID 또는 이름}` |
| `docker exec` | 현재 실행 중인 컨테이너 내부에서 특정 명령어를 추가로 실행 | `docker exec {옵션} {컨테이너 ID 또는 이름} {명령어}` |

- `docker run`에서 사용할 수 있는 옵션 중 일부는 다음과 같다.
    ...
    <details>
    <summary><font color='#0000FF'>옵션 설명</font></summary>
    <div markdown="1">
...
- `docker rm`은 중지된 컨테이너를 내 컴퓨터에서 완전히 삭제하는 명령어다.
    ...
    `-f` 옵션을 사용하면, 실행 중인 컨테이너를 즉시 중지시킨 후 삭제까지 한 번에 처리한다.

- `docker logs`는 컨테이너의 실시간 로그를 출력하는 명령어다.

    `docker run -d`로 컨테이너를 실행하면 터미널 화면이 백그라운드로 숨어버리기 때문에, 로그를 터미널에서 확인하기 위해 해당 명령어를 사용한다.
    <br>
    로그 화면을 빠져나오려면 `Ctrl + C`를 입력하면 된다.

    `-f` 옵션을 사용하면, 로그가 한 번만 찍히고 끝나는 게 아니라 실시간으로 새로 나오는 로그들을 계속 화면에 출력한다.
