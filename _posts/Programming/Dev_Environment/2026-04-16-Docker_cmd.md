...
| `docker images` | 현재 존재하는 도커 이미지들의 전체 목록과 용량을 보여줌 |
...
- `docker build -t {빌드할 이미지 이름} {Dockerfile 경로}`
    ...
    또는 내가 작성한 도커파일을 바탕으로 새로운 이미지를 빌드할 수도 있다.
...
| `docker ps` | 현재 실행 중인 컨테이너들의 목록, 상태, 생성 시간 등을 보여줌 |
...
- `docker run {옵션} {사용할 이미지 이름}`
    ...
    | :---: | :---: |
    | `* -d` | 백그라운드에서 조용히 실행 |
    | `-p` | 내 컴퓨터의 포트와 컨테이너의 포트를 연결 |
    | `--name` | 생성될 컨테이너에 원하는 이름을 붙여 |
...
- `docker stop {컨테이너 ID 또는 이름}`
    ...
    실행 중인 컨테이너를 안전하게 중지합니다. (컴퓨터 전원을 끄는 것과 비슷합니다.)

- `docker rm {컨테이너 ID 또는 이름}`

    중지된 컨테이너를 내 컴퓨터에서 완전히 삭제합니다.
