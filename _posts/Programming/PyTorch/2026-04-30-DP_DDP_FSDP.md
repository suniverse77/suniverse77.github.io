...
하지만 앞서 말했듯이 DDP는 모든 GPU에 모델을 그대로 복제하는 방식이기 때문에, 모델 크기가 단일 GPU의 메모리 용량을 초과하면 학습 자체가 불가능하다는 한계가 있다.
...
- 파라미터 + 그래디언트 + 옵티마이저 상태: 36GB (sharding 가능)
- activation + 기타 버퍼: 12GB (sharding 불가)
