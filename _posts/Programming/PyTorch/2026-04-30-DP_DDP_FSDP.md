...
1. 초기화 단계에서 전체 모델의 파라미터, 옵티마이저 상태, 그래디언트를 GPU 개수만큼 조각 내어, 각 GPU는 자신에게 할당된 파츠만 메모리에 유지한다. 
    <br>
    이렇게 하나의 layer 파라미터 자체를 조각 내는 과정을 sharding이라고 한다.
...
5. 현재 layer의 연산을 수행하는 동시에, 다음 layer의 파라미터 조각들을 미리 모으기 시작한다.
...
![fig8](Programming/PyTorch/DP_DDP_FSDP-8.png){: style="display:block; margin:0 auto; width:40%;"}
