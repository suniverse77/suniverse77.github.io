...
따라서 모델 자체는 GPU 1대에 충분히 올라가지만 Batch Size가 커서 OOM이 발생하는 경우에는 해결이 가능하지만, 모델 자체가 커서 OOM이 발생하는 경우에는 해결하지 못한다.
...
![fig1](Programming/PyTorch/DDP-1.png){: style="display:block; margin:0 auto; width:90%;"}
