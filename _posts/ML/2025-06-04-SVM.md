...
$$
\min_{\mathbf{w},b}{\frac{\lVert\mathbf{w}\rVert^2}{2}}
$$

이때, 제약 조건은 각 데이터 샘플이 올바르게 분리되도록 $y_i(\mathbf{w}^\top\mathbf{x}_i + b) \ge 1$을 만족해야 한다.
...
$$
\begin{aligned}
\min_{\mathbf{w},b,\xi}{\frac{\lVert\mathbf{w}\rVert^2}{2}}+C\sum_i\xi_i
~~~~~~~~\\
\text{subject to}~~y(\mathbf{w}^\top\mathbf{x}+b)\geq1-\xi_i\\
\xi_i\geq0~~~~~~~~~~~~~~~~~~~~~~~~~
\end{aligned}
$$

![fig3](ml/4-3.png){: style="display:block; margin:0 auto; width:40%;"}
_[[출처]](https://www.surveypractice.org/article/2715-using-support-vector-machines-for-survey-research)_
