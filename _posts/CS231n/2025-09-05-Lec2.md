...
고양이의 자세, 카메라의 각도, 빛의 세기 등 과 같은 작은 변화에도 픽셀값들이 변하기 때문에 미리 정해진 규칙으로 이미지를 분석하는 것은 어렵다.

따라서 대규모의 이미지 데이터와 해당 이미지의 레이블을 컴퓨터에 학습시켜 스스로 분류 규칙을 형성하게 하는 접근법이 필요하며, 이를 data-driven approach라고 부른다.

이미지 분류를 위한 data-driven approach에는 Nearest Neighbor Classifier와 Linear Classifier가 있다.

## Nearest Neighbor

Nearest Neighbor는 단순히 , 학습과 예측의 과정은 아래와 같다.
...
<img src="02-1.png" alt="fig1" width="100%">
...
이미지 분류에 NN을 바로 적용하면 위와 같이 픽셀에 바로 distance metric을 적용하기 때문에 의미있는 유사도를 얻지 못한다.
