...
카메라의 내부 파라미터(초점 거리 등)를 알고 있을 때(Calibrated), 두 이미지 좌표 사이의 기하학적 관계를 나타냅니다.

### Fundamental Matrix

카메라 정보를 모르는 상태에서(Uncalibrated), 순수하게 이미지상의 픽셀 좌표만으로 관계를 나타냅니다. 실무에서 가장 많이 활용되는 행렬입니다.
