...
Odd Polynomial Matrix 함수 $\rho(\cdot)$를 사용한다. 이 함수는 행렬을 입력받아 아래와 같은 연산을 수행한다.

$$
\rho(X)=aX+b(XX^\top)X
$$

위 함수 $\rho$는 행렬 전체를 복잡하게 변형시키는 것처럼 보이지만, 실제로는 아래 연산과 동치이다.

$$
\rho(X)=U(aS+bS^3)V^\top
$$

다시 말해, 함수 $\rho(\cdot)$는 회전 정보인 $U$와 $V^\top$는 변화시키지 않고, $S$의 특이값들에만 개별적으로 적용되는 것으로 볼 수 있다.
...
![fig1](new/muon-1.gif){: style="display:block; margin:0 auto; width:100%;"}

![fig2](new/muon-2.png){: style="display:block; margin:0 auto; width:100%;"}

![fig3](new/muon-3.png){: style="display:block; margin:0 auto; width:100%;"}

![fig4](new/muon-4.png){: style="display:block; margin:0 auto; width:100%;"}
