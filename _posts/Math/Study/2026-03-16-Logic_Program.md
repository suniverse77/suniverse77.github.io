...
- **Fact:** `사람(철수)` (철수는 사람이다)
- **Rule:** `밥을 먹는다(x)` $\leftarrow$ `사람(x)` (사람이면 밥을 먹는다.)
...
1. 초기 상태 ($I_0$): 초기에는 아무것도 모르는 상태임 (공집합)
    ...
    ```
2. 첫 번째 추론 ($I_1 = T_P(I_0)$): 조건이 필요 없는 기본 Fact가 도출된다.
    ...
    ```
3. 두 번째 추론 ($I_2 = T_P(I_1)$): 알아낸 Fact를 Rule의 조건에 대입한다. 사람(철수)가 참이므로 밥을먹는다(철수)가 도출된다.

    - 새로운 Fact: `밥을먹는다(철수)`
    - 현재 지식 $I_2$: `{사람(철수), 밥을먹는다(철수)}`
    ...
    ```
4. 세 번째 추론 ($I_3 = T_P(I_2)$): 현재 지식을 바탕으로 Rule을 다시 적용해 보지만, 더 이상 새롭게 도출되는 사실이 없다.

    - 현재 지식 $I_3$: `{사람(철수), 밥을먹는다(철수)}`
