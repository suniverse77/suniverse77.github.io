...
수학적으로 동치이므로, 이렇게 생성된 새로운 분포의 평균이 최적값이 된다.
...
업데이트 공식을 칼만 필터라고 부르고, 이 마법 같은 비율 $k$를 **칼만 이득(Kalman Gain)**이라고 부
...
이 변수들을 앞서 유도한 식에 그대로 대입하고 상태 공간과 관측 공간을 맞춰주기 위한 관측 행렬 $H$를 알맞게 끼워 넣으면, 칼만 필터의 보정 수식이 유도된다.
...
- Kalman gain
    ...
    k = \frac{\sigma_0^2}{\sigma_0^2 + \sigma_1^2}
    ~~~\rightarrow~~~
    K_k=\frac{P_t^- H^\top}{H P_t^- H^\top + R}
    $$
