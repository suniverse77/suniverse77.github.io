...
이론적으로라면 내 컴퓨터가 계산한 **예측된 외부 온도** $\hat{T}_{ext}$와 실제 센서로 읽어들인 **측정된 외부 온도** $T_{ext}$는 완벽하게 똑같아야 한다. 하지만 현실에는 수학적 모델이 담아내지 못하는 미세한 변수들이 존재하기 때문에, 이 두 값 사이에는 항상 오차가 발생하게 된다.
...
그리고 외부 온도의 오차가 0에 수렴하는 순간, 서로 연결된 열역학 공식에 의해 모델 내부에서 함께 계산되고 있던 **예측된 내부 온도** $\hat{T}_{in}$이 우리가 알고 싶어 했던 **진짜 내부 온도** $T_{in}$와 완벽하게 일치하게 된다.
...
3. 하지만 수학적 모델은 이상적인 상황을 가지고 설계되었기 때문에, 예측값 $\hat{T}_{ext}$과 측정값 $T_{ext}$에는 항상 오차가 발생하게 된다.
4. 우리는 피드백 루프를 통해, 수학적 모델이 $\hat{T}_{ext}\approx T_{ext}$가 되도록 내부 상태를 계속 보정한다.
5. 마침내 $\hat{T}_{ext}\approx T_{ext}$가 달성되면, 이때 수학적 모델이 계산한 $\hat{T}_{in}$ 또한 현실의 $T_{in}$에 도달하게 된다.
