---
title: "[컴퓨터 비전] 합성곱 신경망 (Convolutional Neural Network)"
date: 2025-02-20 00:00:00 +/-TTTT
...
![fig1](/assets/images/AI_Basics/CV/CNN-1.png){: style="display:block; margin:0 auto; width:90%;"}
...
![fig2](/assets/images/AI_Basics/CV/CNN-2.png){: style="display:block; margin:0 auto; width:70%;"}
_출처: Stanford CS231n, Lecture 5 (Image Classification with CNNs)_

Convolution Layer에서도 마찬가지로, 필터와 입력 이미지의 일부분을 내적해서 하나의 스칼라를 출력한다. 즉, 출력은 해당 필터의 템플릿과 이미지의 특정 부분이 얼마나 일치하는지를 나타내며, 출력된 2D Activation Map (Feature Map)은 이미지의 각 부분이 해당 필터와 얼마나 일치하는지를 나타낸다.

![fig3](/assets/images/AI_Basics/CV/CNN-3.png){: style="display:block; margin:0 auto; width:60%;"}
_출처: Stanford CS231n, Lecture 5 (Image Classification with CNNs)_

FC layer에서 가중치의 행마다 1개의 bias가 존재하였고, Convolution Layer에서는 필터 하나마다 1개의 bias가 존재한다.

![fig4](/assets/images/AI_Basics/CV/CNN-4.png){: style="display:block; margin:0 auto; width:60%;"}
_출처: Stanford CS231n, Lecture 5 (Image Classification with CNNs)_
...
![fig5](/assets/images/AI_Basics/CV/CNN-5.png){: style="display:block; margin:0 auto; width:60%;"}
_출처: Stanford CS231n, Lecture 5 (Image Classification with CNNs)_

내적은 선형 연산자이다. CNN에서 사용하는 컨볼루션 연산도 일종의 내적이기 때문에 여러 layer를 쌓아도 여전히 선형 연산자이다. 따라서 선형 분류기에서처럼 컨볼루션 연산 사이에도 활성 함수를 추가해야 한다.

![fig6](/assets/images/AI_Basics/CV/CNN-6.png){: style="display:block; margin:0 auto; width:60%;"}
_출처: Stanford CS231n, Lecture 5 (Image Classification with CNNs)_
...
![fig7](/assets/images/AI_Basics/CV/CNN-7.png){: style="display:block; margin:0 auto; width:30%;"}
_출처: Stanford CS231n, Lecture 5 (Image Classification with CNNs)_
...
![fig8](/assets/images/AI_Basics/CV/CNN-8.png){: style="display:block; margin:0 auto; width:60%;"}
_출처: Stanford CS231n, Lecture 5 (Image Classification with CNNs)_
...
아래 그림에서 Output의 한 칸은 Input의 7x7칸에 영향을 받고 있으며, 따라서 이 7x7 영역이 출력의 수용 영역이다.

![fig9](/assets/images/AI_Basics/CV/CNN-9.png){: style="display:block; margin:0 auto; width:80%;"}
_출처: Stanford CS231n, Lecture 5 (Image Classification with CNNs)_

결과적으로, CNN의 layer가 깊어질수록 하나의 필터가 커버하는 수용 영역은 점점 넓어진다.

#### 특징맵 (Feature Map)

일반적으로 Convolution Layer를 지날수록 특징맵의 공간 해상도 (Spatial Resolution)는 점점 줄어들고 채널 크기는 증가한다.

그러면 각 Layer의 특징맵에는 어떤 정보가 담기게 될까?

초기 layer의 특징맵은 공간 해상도가 크기 때문에 세밀한 위치 정보를 잘 보존한다. 이때 특징맵에 담기는 특징은 주로 edge, 색상 대비와 같은 저수준의 시각적 패턴 (low-level feature)으로, 이를 의미적으로 거친 (<span style="background-color:#fff5b1">Semantically coarse</span>) 특징이라고 표현한다.

후반 layer로 갈수록 특징맵의 공간 해상도는 줄어들지만, 수용 영역이 넓어지고 채널이 깊어지면서 정보가 압축된다. 이때는 '차', '사람'과 같은 고수준의 개념적 특징 (high-level feature)이 담긴다.
...
![fig10](/assets/images/AI_Basics/CV/CNN-10.png){: style="display:block; margin:0 auto; width:60%;"}
_출처: Stanford CS231n, Lecture 5 (Image Classification with CNNs)_

기본적으로 연산 후 출력의 크기가 $s=2$인 stride와 동일하지만 추가적인 필터가 필요하지 않기 때문에 컴퓨팅 비용이 저렴하다.

![fig11](/assets/images/AI_Basics/CV/CNN-11.png){: style="display:block; margin:0 auto; width:70%;"}
_출처: Stanford CS231n, Lecture 5 (Image Classification with CNNs)_
...
![fig12](/assets/images/AI_Basics/CV/CNN-12.png){: style="display:block; margin:0 auto; width:80%;"}
_출처: Stanford CS231n, Lecture 5 (Image Classification with CNNs)_
