...
예를 들어 입력 이미지의 크기가 $48\times48$일 때, 이를 $16\times16$ 크기의 패치로 나눈다면 총 $N=9$개의 패치가 생긴다.
<br>
이미지는 RGB 3개의 채널을 가지고 있기 때문에, 하나의 패치 $x_i$는 $16\times16\times3$의 형태를 가진다.
<br>
이후 이 패치를 평탄화하여 1차원 벡터 $\mathbf{x}_i\in\mathbb{R}^{768}$로 변환한다. (벡터가 $768$ 차원인 이유는 하나의 패치에 $16\times16\times3=768$개의 픽셀값이 존재하기 때문임)
<br>
이때 픽셀값은 $0\sim255$ 범위를 가지기 때문에, 정규화를 한다.

$$
x_i\in\mathbb{R}^{P\times P \times C}~\to~\mathbf{x}_i\in\mathbb{R}^{(P^2 \times C)}
\tag{1}
$$

이 정규화된 벡터를 바로 인코더의 입력으로 사용하지 않고, 신경망에 통과시켜 $D$ 차원의 Patch Embedding $\mathbf{e}_i$로 변환한다.

$$
\mathbf{e}_i=W_E~\mathbf{x}_i\in\mathbb{R}^D
\tag{2}
$$

이때, 클래스 토큰 (cls token) $\mathbf{e}_{cls}$를 시퀀스의 맨 앞에 추가한다. 이 토큰은 학습 과정에서 전체 이미지를 대표하며, 이후 분류 단계에서 최종적으로 사용된다.

이렇게 얻은 $N+1$개의 패치 임베딩에 각각 위치 임베딩 (positional embedding) $\mathbf{p}_i$를 더해, 모델이 각 패치의 공간적 위치 정보를 인식할 수 있도록 한다.
