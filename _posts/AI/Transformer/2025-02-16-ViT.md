...
\end{bmatrix}\in\mathbb{R}^{P\times P\times C}
,\quad\text{where }x_{row,col}\in\mathbb{R}^{1\times1\times C}
\tag{1}
$$
...
<br>
이 토큰은 학습 과정에서 전체 이미지를 대표하며, 이후 분류 단계에서 최종적으로 사용된다.
...
이 단계는 언어 모델에서 사용되는 Self-Attention과 매커니즘이 거의 동일하다.

먼저 입력 임베딩 $E$에 가중치 행렬 $W_Q,W_K,W_V\in\mathbb{R}^{D\times D}$를 곱해 Query, Key, Value를 만든다.

$$
Q=E\cdot W_Q\in\mathbb{R}^{(N+1)\times D}~~,~~
K=E\cdot W_K\in\mathbb{R}^{(N+1)\times D}~~,~~
