---
title: "[Diffusion] 디퓨전 모델에서의 Guidance"
...
$$
\mathcal{G}_0:=-\nabla_{\mathbf{x}_0}\ell(\hat{\mathbf{x}}_0, \mathbf{c})
\tag{12}
$$
...
> $$\begin{aligned}
    ...
    &=\frac{1}{\alpha_t}\left(I-\sigma_t\nabla_{\mathbf{x}_t}\boldsymbol{\epsilon}_\theta(\mathbf{x}_t,t)\right)
    \end{aligned}$$
> 
> 정리된 수식에서 $\nabla\_{\mathbf{x}_t}\boldsymbol{\epsilon}\_\theta(\mathbf{x}\_t,t)$는 입력 이미지가 $\mathbf{x}\_t$가 변할 때, U-Net이 출력하는 예측 노이즈 맵이 얼마나 변하는지를 나타낸다.
>
