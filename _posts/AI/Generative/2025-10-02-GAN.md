...
문제는 $\log(1-x)$ 함수의 형태에 있다. 이 함수는 $x$가 0에 가까워질수록, 함수의 기울기가 $0$에 수렴한다.
즉, 신경망에서 그라디언트가 매우 작아져 기울기 소실 문제가 발생하게 된다.

$$
f(x)=\log(1-x)~,~ x \to 0 \implies \nabla_x f(x) \approx 0
$$

아래 그래프에서 볼 수 있듯이, $x$가 0에 가까운 왼쪽 영역은 기울기가 매우 평탄합니다.

![fig4](generative/2-4.png){: style="display:block; margin:0 auto; width:70%;"}
_[[출처]](https://danieltakeshi.github.io/2017/03/05/understanding-generative-adversarial-networks/)_
...
![fig5](generative/2-5.png){: style="display:block; margin:0 auto; width:70%;"}
