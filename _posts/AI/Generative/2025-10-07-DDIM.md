...
1. **Forward 정의:** 노이즈를 주입하는 규칙 $q(\mathbf{x}_t \mid \mathbf{x}\_{t-1})$을 Markov Chain으로 먼저 정의함
2. **Marginal 도출:** 수식을 전개해 보니, 임의의 $t$ 시점에서의 상태 $q(\mathbf{x}_t \mid \mathbf{x}_0)$가 도출됨
3. **Posterior 도출:** 베이즈 정리를 이용해, Reverse Process의 정답 분포 $q(\mathbf{x}_{t-1} \mid \mathbf{x}_t, \mathbf{x}_0)$를 도출함

#### DDIM의 전개 순서 (역방향 $\to$ 정방향)

1. **Marginal 고정:** DDPM에서 쓰던 Marginal 분포 $q(\mathbf{x}_t \mid \mathbf{x}_0)$를 그대로 가져옴
2. **Posterior 설계:** Marginal 분포를 유지하면서 Non-Markovian 목표를 만족하기 위해, Reverse Process의 정답 분포 $q(\mathbf{x}_{t-1} \mid \mathbf{x}_t, \mathbf{x}_0)$가 어떤 모양이어야 하는지 수학적으로 역산해서 식 (4)를 도출함
3. **Forward 확인:** Marginal 분포 $q(\mathbf{x}\_t \mid \mathbf{x}_0)$, $q(\mathbf{x}\_{t-1} \mid \mathbf{x}_0)$ 2개와 식 (4)의 Posterior 가 모두 준비됨
    <br>
...
![fig3](AI/Generative/DDIM-3.png){: style="display:block; margin:0 auto; width:60%;"}
