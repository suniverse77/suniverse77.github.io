...
![fig1](/assets/images/AI_Basics/DL/PEFT-1.png){: style="display:block; margin:0 auto; width:70%;"}
...
이후 Residual Connection을 통해 Adapter를 통과하기 전의 입력값 $x$를 Adapter를 통과한 출력에 더하여, 원본 모델의 지식을 보존한다.

$f$를 비선형 함수라고 할때, Adapter 모듈은 아래와 같이 표현할 수 있다.

$$
\text{Adapter}(x)=\text{Up}\left(f\left(\text{Down}(x)\right)\right)
\tag{1}
$$

최종적으로 Adapter가 들어간 네트워크 구조를 아래와 같이 표현할 수 있다.
...
![fig2](/assets/images/AI_Basics/DL/PEFT-2.png){: style="display:block; margin:0 auto; width:70%;"}
...
사전 학습된 가중치를 $W_0$라고 표현할 때, 파인튜닝 과정은 기존 가중치에 새로운 변화량 $\Delta W$를 더하는 형태로 표현할 수 있다.
...
만약 모델 전체를 업데이트하는 Full Fine-Tuning을 한다면, $\Delta W$는 $W_0$와 동일한 크기의 거대한 행렬이 되어 계산량과 메모리 사용량이 매우 커진다.

LoRA는 파인튜닝할 때 <span style="background-color:#fff5b1">업데이트되는 가중치 변화량 $\Delta W$가 실제로는 복잡하지 않고 매우 단순한 Low-Rank 구조를 가질 것이라는 가정</span>에서 출발하였다.
<br>
이러한 가정은 LLM이 이미 방대한 언어 지식을 보유하고 있고, 파인튜닝은 새로운 지식을 다시 학습하는 것이 아니라 기존의 지식을 특정 목적에 맞게 조율만하는 과정이라는 아이디에서 기반하였다.

LoRA는 $\Delta W$를 직접 학습하는 대신, 이를 두 개의 훨씬 작은 Low-Rank 행렬 $B$와 $A$의 곱으로 쪼개서 학습한다.

$$
\Delta W=BA,\quad\text{where }
W_0\in\mathbb{R}^{d\times k},~B\in\mathbb{R}^{d\times r},~A\in\mathbb{R}^{r\times k}
\tag{4}
$$

이때 $r$은 $r\leq\min(d,k)$를 만족하는 매우 작은 값으로 설정한다.

행렬 곱에 대해 $\text{rank}(BA)\leq\min(\text{rank}(B),\text{rank}(A))$라는 성질이 성립하므로, $\Delta W$의 Rank 또한 $r$ 이하가 된다.

따라서 파인튜닝 수식이 다음과 같이 변환된다.
...
파인튜닝 과정에서 원본 가중치 $W_0$는 동결하고 $A$와 $B$ 행렬만 학습하기 때문에, 학습 파라미터 수가 $d\times k$개에서 $r(d+k)$개로 줄어든다.
...
![fig3](/assets/images/AI_Basics/DL/PEFT-3.png){: style="display:block; margin:0 auto; width:70%;"}
...
![fig4](/assets/images/AI_Basics/DL/PEFT-4.png){: style="display:block; margin:0 auto; width:70%;"}
...
![fig5](/assets/images/AI_Basics/DL/PEFT-5.png){: style="display:block; margin:0 auto; width:70%;"}
